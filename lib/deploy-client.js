...
function trimRevisions(tableName, revisions) {
  const oldKeys = revisions
    .filter(_ref => !_ref.active)
    .slice(this.maxRecentUploads)
    .map(_ref => _ref.revision);
...
module.exports = CoreObject.extend({
  ...
  init(_ref) {
    const
      allowOverwrite = _ref.allowOverwrite,
      maxRecentUploads = _ref.maxRecentUploads,
      knex = _ref.knex,
      client = _ref.client,
  ...
  upload(_ref) {
    ...
    let revisionKey = _ref.revisionKey;
  ...
  activeRevisionKey(_ref) {
    const tableName = _ref.tableName;
